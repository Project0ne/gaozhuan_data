...
color_data_fieldnames = ["color_data_id","name", "lego_color_id", "font-color", "color", "colorType", "ldraw_color_id", "ldraw_color_value", "index", "name_en"]
# 合并所有的字段名
...
def fetch_data(id):
    ...
    for d in data:
        ...
        #用于调整idraw的数量if ',' in filtered_data[-1]['ldraw_no']:#
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    # 将请求提交给执行器并将数据存储在data列表中
    for result in executor.map(fetch_data, ids):
        data.extend(result)
    # 关闭进度条
    pbar.close()
    # 把data列表转换为一个pandas的DataFrame，并保存为xlsx文件
